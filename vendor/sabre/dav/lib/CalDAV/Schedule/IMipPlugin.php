...
class IMipPlugin extends DAV\ServerPlugin
{
    ...
    public function schedule(ITip\Message $iTipMessage)
    {
        ...
        if (!$iTipMessage->significantChange) {
            ...
            // changed 源码这里只通知重大更新，2025年2月28日 完全放开更新所有时间
//            return;
        }
        ...
        switch (strtoupper($iTipMessage->method)) {
            case 'REPLY':
                ...
                foreach ($iTipMessage->message->VEVENT->ATTENDEE as $attendee) {
                    if (strtolower($attendee->getValue()) == $iTipMessage->sender) {
                        switch ($attendee['PARTSTAT']) {
                            ...
                            case "TENTATIVE":
                                ...
                                break;
                            default:
                                $replayPrefix = "回复 ".$attendee['PARTSTAT']." : ";
                                break;
                        }
                        break;
                    }
                }
                $subject = $replayPrefix.$summary;
                break;
            case 'REQUEST':
                $subject = '邀请: '.$summary;
                break;
